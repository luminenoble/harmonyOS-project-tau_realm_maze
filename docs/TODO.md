...
### 散热系统（`HeatManager.ts`）
- [ ] 新建 `game/core/HeatManager.ts`：维护每层独立热量值（0–100）
- [ ] 玩家在同层每步 +热量；底层（Layer 0）积热速度 ×2（器件层模拟）
- [ ] 踩踏散热通道（`THERMAL_VIA` 瓦片）：当前层热量 -30
- [ ] 热量 ≥ 80 触发"过热减速"（每步等待额外帧），≥ 100 强制弹出当前层（降层）
- [ ] HUD 新增热量条：蓝→黄→红渐变，配芯片风格样式
- [ ] `MapManager` 每层随机放置 2–3 个 `THERMAL_VIA` 散热节点
- [ ] 验收标准：散热条实时响应，过热弹层可复现，底层明显比顶层热
