# TauRealmMaze - 开发工作流程（10天计划）

> AI 辅助开发，目标：HarmonyOS ArkTS + Canvas 实现 2.5D 等距迷宫解谜游戏
>
> **工程路径**：`D:\wsl-share\harmony-game\app-storage\`（WSL：`/mnt/d/wsl-share/harmony-game/app-storage/`）
> **编译执行**：DevEco Studio（Windows）负责，由用户进行；Claude Code（WSL）负责代码编写

---

## Day 1 — 环境搭建 & 等距渲染验证 ⚡关键里程碑 ✅ 已完成（2026-05-26）

**目标：** 在 Canvas 上渲染出一块静态等距菱形地图，确认技术路线可行。

- [x] DevEco Studio 确认工程已建立于 `app-storage\`，应用名称设为 `TauRealmMaze`，包名建议 `com.taorealmmaze.app`
- [x] 确认 WSL 可访问 `/mnt/d/wsl-share/harmony-game/app-storage/entry/src/main/ets/`（`ls` 验证）
- [x] 新建 `GamePage.ets`，挂载 Canvas 组件
- [x] 实现 `IsoMath.ts`：网格坐标 ↔ 屏幕坐标互转
- [x] 用 Canvas 绘制 5×5 静态等距菱形地图（地板瓦片）
- [x] 验收标准：菱形网格正确显示，无坐标错位
- [x] 附加：`Index.ets` 改造为主菜单 + 路由跳转；`IsoRenderer.ts` 抽出绘制函数；`docs/day1/day1-plan.md` 内部蓝图；git 仓库初始化并合并远端 MIT LICENSE

> **编译检测方式**：Claude Code 写完代码 → 你在 DevEco Studio `Build → Make Module` → 报错截图/粘贴给 Claude Code → 循环修复
>
> ⚠️ 如果渲染效果不理想，Day 2 早上决定是否切换降级方案（分层2D俯视图）

---

## Day 2 — 迷宫生成 & 地图数据结构

**目标：** 实现随机迷宫生成，并在等距视角下正确渲染墙体。

- [ ] 实现 `MazeGenerator.ts`（递归回溯算法，输出二维网格通道数组）
- [ ] 定义瓦片类型枚举：`FLOOR / WALL / VIA / GATE / FRAGMENT`
- [ ] 实现 `TileSet.ts`：各瓦片的 Canvas 绘制方法（几何图形，无需图片资源）
- [ ] 将迷宫数据渲染到等距视图，包含墙体遮挡排序（Painter's Algorithm）
- [ ] 验收标准：随机迷宫每次不同，墙体前后遮挡关系正确

---

## Day 3 — 玩家移动 & 输入控制

**目标：** 玩家可以在地图内移动，碰撞检测正常。

- [ ] 实现 `Player.ts`：位置状态、移动方法、动画帧
- [ ] 在 Canvas 上绘制玩家角色（简单几何体，如青绿色菱形）
- [ ] 实现触摸/滑动输入：上下左右四方向移动（可选：屏幕虚拟方向键）
- [ ] 碰撞检测：阻止玩家穿越墙体
- [ ] 实现 `GameEngine.ts` 主循环（requestAnimationFrame 模式）
- [ ] 验收标准：玩家在迷宫内流畅移动，不穿墙

---

## Day 4 — 多层地图 & 层间跳跃（Via 节点）⚡关键里程碑

**目标：** 实现 3 层地图结构，玩家可通过 Via 节点上下切层。

- [ ] 扩展 `MapManager.ts`：管理 3 层独立迷宫网格
- [ ] 在每层地图上放置 Via 节点（上行/下行区分）
- [ ] 实现层切换逻辑：玩家踩上 Via → 全屏淡入淡出 → 切换到相邻层
- [ ] 切层时保持玩家在新层的合法起始位置
- [ ] 实现 `UIOverlay.ts`：HUD 显示当前层数（Layer 0/1/2）
- [ ] 验收标准：3层地图可互相切换，层数 HUD 正确更新

---

## Day 5 — 解谜元素：信号碎片 & 逻辑门

**目标：** 加入核心解谜内容，游戏有了目标和交互。

- [ ] 在地图随机位置放置"信号碎片"道具（每层 3 个）
- [ ] 玩家走到碎片位置自动拾取，HUD 计数更新
- [ ] 实现 `GateLogic.ts`：逻辑门障碍，需集齐指定数量碎片才能通过
- [ ] 实现 `ViaUnlock.ts`：部分 Via 节点默认锁定，满足条件后解锁
- [ ] 简单文本弹窗（ArkUI Dialog）：拾取碎片时显示芯片知识一句话科普
- [ ] 验收标准：收集碎片 → 解锁逻辑门 → 通向下一区域，流程完整

---

## Day 6 — 动态地图（移动迷宫机制）⚡关键里程碑

**目标：** 每 20 步触发一次地图局部重构，增加紧张感。

- [ ] 在 `MapManager.ts` 中实现步数计数器
- [ ] 设计局部重构算法：随机封闭/开启若干通道（保证地图仍可解）
- [ ] 重构前播放警告动效（红色闪烁 + 提示文字"信号重路由中..."）
- [ ] 重构后播放青绿脉冲动效，新通道高亮显示
- [ ] 确保重构后玩家位置仍合法（玩家所在格不变为墙）
- [ ] 验收标准：动态重构不导致死局，视觉反馈清晰
- [ ] **可选难度强化：等距视角旋转（90° 四档）**
  - 在 `IsoRenderer` 层做坐标预变换（`rotation: 0|1|2|3`），`MapManager` 不感知
  - 玩家输入向量按同样矩阵旋转
  - 配套 ~300ms tween 过渡动画 + HUD 罗盘指示
  - 与动态重构叠加产生方向迷失感；若动态重构难度已足够，可推迟到 Day 8 视觉润色阶段

---

## Day 7 — 主菜单 & 结算页面

**目标：** 完善游戏完整流程，有始有终。

- [ ] 实现 `Index.ets` 主菜单：游戏标题、开始按钮、简介文字（融入韬定律背景）
- [ ] 实现通关条件：收集全部碎片 + 到达顶层（Layer 2）指定终点
- [ ] 实现 `ResultPage.ets`：通关画面（步数统计）、失败画面（可选：步数上限）
- [ ] 页面路由配置：菜单 → 游戏 → 结算 → 返回菜单
- [ ] 验收标准：完整游戏流程可跑通一遍

---

## Day 8 — 视觉润色 & 音效（可选）

**目标：** 提升游戏质感，让演示效果更好看。

- [ ] 统一视觉风格：深蓝/黑底 + 青绿电路线条
- [ ] 为瓦片绘制添加电路板细节（线条装饰）
- [ ] 层切换动画优化（丝滑过渡）
- [ ] （可选）使用 HarmonyOS AudioRenderer 添加背景氛围音效
- [ ] （可选）玩家移动音效（短促的电子音）
- [ ] UI 字体、颜色、间距整体微调

---

## Day 9 — 测试 & Bug 修复

**目标：** 全面测试，消灭主要 Bug。

- [ ] 全流程通关测试（正常路径）
- [ ] 压力测试：动态重构触发边界情况（玩家贴墙时重构）
- [ ] 层间切换异常测试（快速连续切层）
- [ ] 不同屏幕分辨率适配检查（Canvas 缩放）
- [ ] 修复所有阻断游戏流程的 Critical Bug
- [ ] 修复视觉错位、遮挡排序错误

---

## Day 10 — 打包 & 演示准备

**目标：** 输出可演示的安装包，准备课程答辩材料。

- [ ] 最终代码整理，删除调试日志
- [ ] 配置 `module.json5`：应用名称、图标（芯片风格）
- [ ] DevEco Studio 打包为 `.hap` 文件
- [ ] 在真机/模拟器上完整演示一遍
- [ ] （可选）录制 30 秒演示视频
- [ ] 整理一页 README：项目说明 + 韬定律概念映射 + 操作指南

---

## 关键里程碑总览

| 天数 | 里程碑 | 风险 |
|------|--------|------|
| Day 1 | 等距渲染跑通 | ⚠️ 高（技术验证） |
| Day 4 | 3层地图+Via切换 | 中 |
| Day 6 | 动态地图稳定 | 中 |
| Day 7 | 完整游戏流程 | 低 |

## Claude Code 使用规范（WSL 内）

```bash
# Claude Code 启动路径（每次开发前 cd 到此）
cd /mnt/d/wsl-share/harmony-game/app-storage

# 常用文件路径前缀
entry/src/main/ets/pages/       # ArkUI 页面
entry/src/main/ets/game/        # 游戏逻辑
entry/src/main/ets/utils/       # 工具函数
entry/src/main/resources/       # 资源文件
```

Claude Code 只负责读写 `.ets` / `.ts` / `.json5` 文件，**不执行任何编译命令**。编译统一由你在 DevEco Studio（Windows）执行。

## 降级策略

- **等距渲染失败** → Day 2 切换分层2D俯视图，节省2天时间补其他功能
- **动态地图死局** → 简化为仅开启新通道，不封闭已有通道
- **音效时间不够** → Day 8 跳过，直接进 Day 9
